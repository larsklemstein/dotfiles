...
-- configure Ruff CLI args (ignore syntax + undefined name)
lint.linters.ruff = {
    cmd = "ruff",
    stdin = false,
    args = { "check", "--quiet", "--ignore=E999,F821", "--stdin-filename", "$FILENAME", "-" },
    stream = "stdout",
    parser = require("lint.parser").from_errorformat("%f:%l:%c: %t%n %m", {
        source = "Ruff",
        severities = { E = vim.diagnostic.severity.ERROR, W = vim.diagnostic.severity.WARN },
    }),
}

lint.linters_by_ft = {
    python = { "ruff" }, -- Ruff diagnostics come only from nvim-lint now
    sh = { "shellcheck" },
    yaml = { "yamllint" },
}

-- Optional: trigger lint on save/changes
